...
- (BOOL)isEqual:(id)other {
    if([other isKindOfClass:[self class]]) {
		...
		if([_elementKind isEqual:otherLayoutAttributes.elementKind] &&
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p frame:%@ indexPath:%@ elementKind:%@>",
			NSStringFromClass([self class]), self, NSStringFromRect(self.frame), self.indexPath, self.elementKind];
}
...
// return YES to requery the layout for geometry information
...
- (void)prepareForCollectionViewUpdates:(NSArray *)updateItems {
    ...
    for (TUICollectionReusableView *view in [[_collectionView visibleViewsDict] objectEnumerator]) {
        ...
        if(index != NSNotFound) {
            ...
            if(index != NSNotFound) {
                ...
                [_initialAnimationLayoutAttributesDict setObject:attr forKey:[TUICollectionViewItemKey collectionItemKeyForLayoutAttributes:attr]];
            }
        }
    }
    ...
    for (TUICollectionViewLayoutAttributes *attr in [collectionViewData layoutAttributesForElementsInRect:bounds]) {
        ...
        if(index != NSNotFound) {
            ...
            [_finalAnimationLayoutAttributesDict setObject:finalAttrs forKey:[TUICollectionViewItemKey collectionItemKeyForLayoutAttributes:finalAttrs]];
        }
    }
