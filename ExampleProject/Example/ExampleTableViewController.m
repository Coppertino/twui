...
- (TUIView *)tableView:(TUITableView *)tableView headerViewForSection:(NSInteger)section {
	...
	TUITextStorage *storage = [TUITextStorage storageWithString:[NSString stringWithFormat:@"Example Section %d", (int)section]];
	storage.color = [NSColor blackColor];
	storage.font = [NSFont fontWithName:@"HelveticaNeue-Bold" size:15];
	header.labelRenderer.textStorage = storage;
...
- (TUITableViewCell *)tableView:(TUITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	TUITextStorage *s = [TUITextStorage storageWithString:[NSString stringWithFormat:@"example cell %ld", indexPath.row]];
